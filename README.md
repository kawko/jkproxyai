...
+-------------------------------+
        ...
        v
+----------+---------+---------+---------+-----------+-----------+---------+
|OpenRouter| Kilo AI | Google  |  Groq   | Cerebras  | SambaNova | Mistral |
|  (ฟรี)   |  (ฟรี)  |AI Studio|  (ฟรี)  |   (ฟรี)   |   (ฟรี)   |  (ฟรี)  |
+----------+---------+---------+---------+-----------+-----------+---------+
```
...
- ระบบ fallback ไปโมเดลอื่นทันที (สูงสุด 7 ครั้ง กระจายข้าม provider แบบ round-robin)
...
| `X-BCProxy-Provider` | ผู้ให้บริการ (openrouter/kilo/groq/cerebras/sambanova/mistral) |
...
| GET | `/api/gateway-logs` | Gateway request/response logs (realtime) |
...
| GET | `/api/health` | Health check — status healthy/degraded/down + alerts |
...
- ดึงรายชื่อโมเดลฟรีจาก **7 ผู้ให้บริการ**พร้อมกัน (OpenRouter, Kilo, Google, Groq, Cerebras, SambaNova, Mistral)
...
| **แข่งความเร็ว** | Speed Race animation — providers แข่งกัน + โพเดียม 🥇🥈🥉 |
| **ห้องเรียน AI** | Grid แสดงทุกโมเดล + เกรด (A+ ถึง F) + ชื่อเล่นไทย + สถานะตลก |
| **ทดลองแชท** | ทดสอบแชทกับโมเดลที่เลือกได้ |
| **Gateway Log** | LIVE log ทุก request เข้า-ออก (refresh ทุก 2 วินาที) |
| **บันทึกการทำงาน** | Log การทำงานของ worker |

Dashboard รีเฟรชอัตโนมัติทุก 15 วินาที (Gateway Log ทุก 2 วินาที)

### ห้องเรียน AI (Model Grid)

โมเดลแต่ละตัวแสดงผลแบบ "นักเรียน" พร้อม:
- **ชื่อเล่นภาษาไทย** สีทอง (DeepSeek ตั้งให้)
- **เกรด**: A+ (97%+) นักเรียนดีเด่น, A (80%+) เก่งมาก, ... F ไม่ผ่าน
- **สถานะตลก**: หัวหน้าห้อง!, ไปพักผ่อนก่อน, รอเข้าห้องสอบ, ซ้ำชั้นเลย
- **ความเร็ว**: สายฟ้า, เร็วมาก, ช้ามาก
...
รองรับ API Key หลายตัวต่อ provider — ใส่คั่นด้วยจุลภาค:
...
แสดง: ค่า GPT-4o vs Claude Sonnet vs Qwen Plus vs BCProxyAI (ฟรี!) + ยอดสะสม + เงินบาท

### Speed Race (แข่งความเร็ว)

Dashboard แสดง animation แข่งความเร็วระหว่าง providers:
- 🏎️ Groq vs 🚀 SambaNova vs 🧠 Cerebras vs 💨 Mistral vs ...
- เรียงตาม latency จริง + โพเดียม 🥇🥈🥉
...
- ถ้าทุกตัว cooldown → สุ่มจากทั้งหมด (ดีกว่า 503)
...
> ถ้าต้องการรองรับมากกว่านี้ ต้องใช้ paid API
...
- **ค่า Claude Code** -- Claude Pro/Max subscription (ใช้สร้างโปรเจค)
- **ค่า AI Model** -- ฟรี! ใช้เฉพาะโมเดลฟรี 7 providers
- **ค่า Server** -- ฟรี! รันบน Docker Desktop บนเครื่องตัวเอง

---

## ค่าใช้จ่ายในการรัน (ต่อวัน)
...
|--------|---------|-------------------|
| ตรวจข้อสอบ (judge) | 9 calls/ชม. × 24 = 216 calls/วัน | ~$0.22/วัน (~฿8) |
| ตั้งชื่อเล่น (model ใหม่) | 10 ชื่อ/รอบ (เฉพาะวันแรก) | ~$0.12 ครั้งเดียว |
| ตั้งชื่อหลังสอบ | 3 ชื่อ/รอบ | ~$0.04/วัน (~฿1) |
| **รวม** | | **~$0.26/วัน (~฿9)** |

> **หลังสแกนครบทุกตัวแล้ว:** ค่าใช้จ่ายลดเหลือ ~$0.05/วัน (~฿2) เพราะตั้งชื่อเฉพาะ model ใหม่เท่านั้น
...
**7 Providers** | **130+ โมเดลฟรี** | **Auto-Fallback** | **Speed Race** | **Thai Benchmark** | **Fun Nicknames** | **Health Monitoring** | **In-Memory Cache**
